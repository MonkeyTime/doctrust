...
def resolve_trust_anchor_public_key(registry, issuer):
    ...
    if anchor.get("status") and anchor.get("status") != "active":
        raise ValueError(f"Trust anchor is not active: {issuer['trust_anchor_id']}")
    if anchor.get("issuer_id") and anchor.get("issuer_id") != issuer["issuer_id"]:
    ...
    current_key = next((item for item in public_keys if isinstance(item, dict) and item.get("status") != "revoked"), None)
